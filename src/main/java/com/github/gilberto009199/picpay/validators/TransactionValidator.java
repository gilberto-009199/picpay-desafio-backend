...
public class TransactionValidator {
    ...
    public static boolean validateCreateTransaction(TransactionEntity transaction, WalletEntity walletPayer, WalletEntity walletPayee){
        ...
        if(transaction.getBalance() == null || transaction.getBalance().compareTo(BigDecimal.ZERO) <= 0) return false;

        // 3. Não pode transferir se a carteira pagadora (walletPayer) não tiver saldo suficiente
        if(walletPayer.getBalance() == null || walletPayer.getBalance().compareTo(transaction.getBalance()) < 0)return false;
        ...
        if(transaction.getBalance().scale() > 2)return false;
